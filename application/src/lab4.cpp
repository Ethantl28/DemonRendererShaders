...
Lab4::Lab4(GLFWWindowImpl& win) : Layer(win)
{
	...
	//Shader description
	ShaderDescription floorShaderDesc = CreateShaderDesc(ShaderType::rasterization, "./assets/shaders/Lab1/floorVertex.glsl", "./assets/shaders/lab4/gBufferFloorFrag.glsl");
	...
	skyboxPassIdx = m_mainRenderer.getRenderPassCount();
	...
	lightPassMaterial->setValue("u_depth", mainPass.target->getTarget(3));
	...
	lightPass.UBOmanager.setCachedValue("b_camera2D", "u_viewPos", m_mainScene->m_actors.at(cameraIdx).translation);
	...
	lightPassIdx = m_mainRenderer.getRenderPassCount();
	...
	blurMaterial->setValue("u_inputTexture", lightPass.target->getTarget(0));
	...
	dofMaterial->setValue("u_inFocus", lightPass.target->getTarget(0));
...
void Lab4::onUpdate(float timestep)
{
	...
	auto& lightPassMat = m_mainRenderer.getRenderPass(lightPassIdx).scene->m_actors.at(0).material;
	lightPassMat->setValue("u_pLightsAmbient", m_pointLightsAmbient);
	lightPassMat->setValue("u_pLightsShine", m_pointLightsShine);
	...
	lightPass.UBOmanager.setCachedValue("b_camera", "u_view", lightPass.camera.view);
...
ShaderDescription Lab4::CreateShaderDesc(ShaderType type, std::string vertexPath, std::string fragPath)
{
...
std::shared_ptr<Shader> Lab4::CreateShader(ShaderDescription shaderDesc)
{
...
std::shared_ptr<VAO> Lab4::CreateVAO(std::vector<unsigned int> indicies, std::vector<float> vertices, VBOLayout layout)
{
...
Actor Lab4::CreateActor(std::shared_ptr<VAO> Vao, std::shared_ptr<VAO> depthVao, std::shared_ptr<Material> material,
...
int Lab4::AddActorToRender(Actor actor)
{
